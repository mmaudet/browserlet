...
describe('BSLRunner', { timeout: 60000 }, () => {
  ...
  it('runs simple.bsl successfully with exit code 0', async () => {
    ...
    const runner = new BSLRunner(page, context, { globalTimeout: 15000, outputDir: path.join(os.tmpdir(), 'browserlet-test-output') });
...
`;
    ...
    try {
      ...
      const runner = new BSLRunner(page, context, { globalTimeout: 5000, outputDir: path.join(os.tmpdir(), 'browserlet-test-output') });
  ...
  it('returns exit code 1 on missing selector (timeout.bsl)', async () => {
    ...
    const runner = new BSLRunner(page, context, { globalTimeout: 2000, outputDir: path.join(os.tmpdir(), 'browserlet-test-output') });
    const result = await runner.run(scriptPath);
