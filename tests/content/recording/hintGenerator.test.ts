...
import { looksLikeHashedClass, looksAutoGenerated, generateHints, detectSPAContext } from '../../../entrypoints/content/recording/hintGenerator';
...
// ---------------------------------------------------------------------------
// generateHints - position_context
// ---------------------------------------------------------------------------

describe('generateHints - position_context', () => {
  it('should NOT emit position_context when element text is unique', () => {
    document.body.innerHTML = `
    ...
    const hints = generateHints(btn);
    const pos = hints.find(h => h.type === 'position_context');
    expect(pos).toBeUndefined();
  });

  it('should emit position_context when multiple buttons have same text', () => {
    document.body.innerHTML = `
      <button id="e1">Edit</button>
      <button id="e2">Edit</button>
      <button id="e3">Edit</button>
    `;
    const btn = document.getElementById('e2')!;
    const hints = generateHints(btn);
    const pos = hints.find(h => h.type === 'position_context');
    expect(pos).toBeDefined();
    expect(pos?.value).toBe('item 2 of 3');
  });

  it('should emit row-based position_context for table buttons', () => {
    document.body.innerHTML = `
      <table>
        <tbody>
          <tr><td><button>Edit</button></td><td>Alice</td></tr>
          <tr><td><button id="target">Edit</button></td><td>Bob</td></tr>
          <tr><td><button>Edit</button></td><td>Carol</td></tr>
        </tbody>
      </table>
    `;
    ...
    const pos = hints.find(h => h.type === 'position_context');
    expect(pos).toBeDefined();
    expect(pos?.value).toBe('row 2 of 3');
  });

  it('should NOT emit position_context for element without text hint', () => {
    document.body.innerHTML = `
...
describe('detectSPAContext', () => {
  it('should return undefined on a plain HTML page with no SPA signals', () => {
    const result = detectSPAContext(document.body);
    expect(result).toBeUndefined();
  });

  it('should detect React when __REACT_DEVTOOLS_GLOBAL_HOOK__ is present', () => {
    (window as Window & typeof globalThis & { __REACT_DEVTOOLS_GLOBAL_HOOK__?: object }).__REACT_DEVTOOLS_GLOBAL_HOOK__ = {};
    const result = detectSPAContext(document.body);
    expect(result).toBeDefined();
    expect(result?.framework).toBe('react');
    expect(result?.is_dynamic_zone).toBe(false);
    delete (window as Window & typeof globalThis & { __REACT_DEVTOOLS_GLOBAL_HOOK__?: object }).__REACT_DEVTOOLS_GLOBAL_HOOK__;
  });

  it('should detect Vue when __VUE__ global is present', () => {
    (window as Window & typeof globalThis & { __VUE__?: boolean }).__VUE__ = true;
    const result = detectSPAContext(document.body);
    expect(result).toBeDefined();
    expect(result?.framework).toBe('vue');
    delete (window as Window & typeof globalThis & { __VUE__?: boolean }).__VUE__;
  });

  it('should detect component from data-component attribute on ancestor', () => {
    document.body.innerHTML = `
      <div data-component="UserTable">
        <button id="btn">Edit</button>
      </div>
    `;
    (window as Window & typeof globalThis & { __VUE__?: boolean }).__VUE__ = true;
    const btn = document.getElementById('btn')!;
    const result = detectSPAContext(btn);
    expect(result?.component).toBe('UserTable');
    delete (window as Window & typeof globalThis & { __VUE__?: boolean }).__VUE__;
  });
});
